defmodule ExRakLib.DataTypes do
  require Logger

  def decode_ip_address(
        <<first::unsigned-size(8), second::unsigned-size(8), third::unsigned-size(8),
          fourth::unsigned-size(8), rest::binary>>
      ),
      do: {{first, second, third, fourth}, rest}

  def encode_ip_address({first, second, third, fourth}),
    do:
      <<first::unsigned-size(8), second::unsigned-size(8), third::unsigned-size(8),
        fourth::unsigned-size(8)>>
  ...
  def decode_string(<<prefix::unsigned-size(16), string::binary-size(prefix), rest::binary>>),
    do: {string, rest}

  def encode_string(string), do: <<byte_size(string)::unsigned-size(16), string::bitstring>>

  def decode_bstring(<<prefix::unsigned-size(8), string::binary-size(prefix), rest::binary>>),
    do: {string, rest}

  def encode_bstring(string), do: <<byte_size(string)::unsigned-size(8), string::bitstring>>

  def decode_address_port(
        <<version::size(8), address::binary-size(4), port::unsigned-size(16), rest::binary>>
      ),
      do: {%{version: 4, address: elem(decode_ip_address(address), 0), port: port}, rest}

  def encode_address_port(%{version: version, address: address, port: port}),
    do: <<4::size(8), encode_ip_address(address)::binary, port::unsigned-size(16)>>

  def decode_data_packet(data, internal \\ false) do
    <<sequence_number::little-size(24), rest::binary>> = :erlang.iolist_to_binary(data)

    %{
      sequence_number: sequence_number,
      encapsulated_packets: decode_data_packet(rest, [], internal)
    }
  end

  defp decode_data_packet("", encapsulated_packets, internal) do
    Enum.reverse(encapsulated_packets)
  end

  defp decode_data_packet(rest, encapsulated_packets, internal) do
    {packet, rest} = decode_encapsulated_packet(rest, internal)
    decode_data_packet(rest, [packet | encapsulated_packets], internal)
  end
  ...
  def encode_data_packet(
    ...
    :erlang.iolist_to_binary([
      <<sequence_number::little-size(24)>>,
      Enum.map(encapsulated_packets, fn x -> encode_encapsulated_packet(x, internal) end)
    ])
  end
  ...
  def encode_encapsulated_packet(options, internal) do
    data =
      if internal do
        [
          <<options[:reliability]::unsigned-size(3), options[:has_split]::unsigned-size(5),
            byte_size(options[:buffer])::size(32), options[:identifier_ack]::size(32)>>
        ]
      else
        [
          <<options[:reliability]::unsigned-size(3), options[:has_split]::unsigned-size(5),
    ...
    data =
      if options[:reliability] in [2, 3, 4, 6, 7] do
    ...
    data =
      if options[:reliability] in [1, 3, 4] do
    ...
    data =
      if options[:has_split] > 0 do
        [
          data,
          <<options[:split_count]::size(32), options[:split_id]::size(16),
            options[:split_index]::size(32)>>
        ]
      else
        [data]
      end

    :erlang.iolist_to_binary([data, options[:buffer]])
  end
end
